...
integer                   :: cont,n,jj !m=dimension of the Hamiltonian (Nq1*Nq2*Nst x Nq1*Nq2*Nst)
...
complex(dp)               :: auto_h(0:nfiles),auto_d(0:nfiles),aux_val
real(dp)                  :: auto_hr(0:nfiles),auto_dr(0:nfiles)
real(dp)                  :: diag_hr(0:nfiles),diag_dr(0:nfiles)
real(dp)                  :: auto_hr_01(0:nfiles),auto_dr_01(0:nfiles)
real(dp)                  :: auto_hr_02(0:nfiles),auto_dr_02(0:nfiles)
real(dp)                  :: auto_hr_12(0:nfiles),auto_dr_12(0:nfiles)
real(dp)                  :: auto_hr_0(0:nfiles),auto_dr_0(0:nfiles)
real(dp)                  :: auto_hr_1(0:nfiles),auto_dr_1(0:nfiles)
real(dp)                  :: auto_hr_2(0:nfiles),auto_dr_2(0:nfiles)
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
!hi=2.01410177811_dp*saw2au   !1.00794_dp*saw2au !Hidrogen mass in atomic units
...
do ppp=1,3  !nsamples    <<<<<====================----------------==================----------------===============----------------================
  ...
  auto_h    = dcmplx(0.0_dp,0.0_dp)

!$omp parallel do reduction(+:diag_hr,auto_h)!!,auto_d)
  do i=0,nfiles
    do j=0,n-1
      auto_h(i) = auto_h(i) + (dconjg(wfout(j,0)) * wfout(j,i))
      diag_hr(i) = diag_hr(i) + dconjg(wfout(j,0))*wfout(j,i) * ((wfout(j,0))*dconjg(wfout(j,i))) * eig_val(ppp)
    end do
  end do
!$omp end parallel do

!$OMP parallel do reduction(+:auto_hr_0,auto_hr_1,auto_hr_2,auto_hr_01,auto_hr_02,auto_hr_12)
  do i = 0,nfiles
    do j=0,s-1
      do k=0,s-1
        if ( j /= k ) then
auto_hr_01(i) = auto_hr_01(i) + wfout(0*s+j,0) * dconjg(wfout(0*s+j,i)) * dconjg(wfout(1*s+k,0)) * wfout(1*s+k,i) * eig_val(ppp) + &
wfout(1*s+j,0) * dconjg(wfout(1*s+j,i)) * dconjg(wfout(0*s+k,0)) * wfout(0*s+k,i) * eig_val(ppp)
auto_hr_02(i) = auto_hr_02(i) + wfout(0*s+j,0) * dconjg(wfout(0*s+j,i)) * dconjg(wfout(2*s+k,0)) * wfout(2*s+k,i) * eig_val(ppp) + &
wfout(2*s+j,0) * dconjg(wfout(2*s+j,i)) * dconjg(wfout(0*s+k,0)) * wfout(0*s+k,i) * eig_val(ppp)
auto_hr_12(i) = auto_hr_12(i) + wfout(1*s+j,0) * dconjg(wfout(1*s+j,i)) * dconjg(wfout(2*s+k,0)) * wfout(2*s+k,i) * eig_val(ppp) + &
wfout(2*s+j,0) * dconjg(wfout(2*s+j,i)) * dconjg(wfout(1*s+k,0)) * wfout(1*s+k,i) * eig_val(ppp)
auto_hr_0(i)  = auto_hr_0(i)  + wfout(0*s+j,0) * dconjg(wfout(0*s+j,i)) * dconjg(wfout(0*s+k,0)) * wfout(0*s+k,i) * eig_val(ppp)
auto_hr_1(i)  = auto_hr_1(i)  + wfout(1*s+j,0) * dconjg(wfout(1*s+j,i)) * dconjg(wfout(1*s+k,0)) * wfout(1*s+k,i) * eig_val(ppp)
auto_hr_2(i)  = auto_hr_2(i)  + wfout(2*s+j,0) * dconjg(wfout(2*s+j,i)) * dconjg(wfout(2*s+k,0)) * wfout(2*s+k,i) * eig_val(ppp)
        else
auto_hr_01(i) = auto_hr_01(i) + wfout(0*s+j,0) * dconjg(wfout(0*s+j,i)) * dconjg(wfout(1*s+k,0)) * wfout(1*s+k,i) * eig_val(ppp) + &
wfout(1*s+j,0) * dconjg(wfout(1*s+j,i)) * dconjg(wfout(0*s+k,0)) * wfout(0*s+k,i) * eig_val(ppp)
auto_hr_02(i) = auto_hr_02(i) + wfout(0*s+j,0) * dconjg(wfout(0*s+j,i)) * dconjg(wfout(2*s+k,0)) * wfout(2*s+k,i) * eig_val(ppp) + &
...
!$OMP end parallel do
  ...
  do i=0,nfiles
    auto_hr(i) = auto_hr(i) + dconjg(auto_h(i)) * auto_h(i) * eig_val(ppp)
  end do
...
open(unit=960,file='autoc_h.txt'   ,status='unknown')
open(unit=961,file='diago_h.txt'   ,status='unknown')
open(unit=962,file='cross_h_01.txt',status='unknown')
open(unit=963,file='cross_h_02.txt',status='unknown')
open(unit=964,file='cross_h_12.txt',status='unknown')
open(unit=965,file='cross_h_0.txt' ,status='unknown')
open(unit=966,file='cross_h_1.txt' ,status='unknown')
open(unit=967,file='cross_h_2.txt' ,status='unknown')
do i = 0,nfiles
  write(960,'(e20.10e3)')auto_hr(i)
  write(961,'(e20.10e3)')diag_hr(i)
  write(962,'(e20.10e3)')auto_hr_01(i)
  write(963,'(e20.10e3)')auto_hr_02(i)
  write(964,'(e20.10e3)')auto_hr_12(i)
  write(965,'(e20.10e3)')auto_hr_0(i)
  write(966,'(e20.10e3)')auto_hr_1(i)
  write(967,'(e20.10e3)')auto_hr_2(i)
end do
close(unit=960)
close(unit=961)
close(unit=962)
...
deallocate(nac_val,nac_rowc,nac_row_col)
deallocate(dipx_val,dipx_rowc,dipx_row_col)
deallocate(dipy_val,dipy_rowc,dipy_row_col)
deallocate(dipz_val,dipz_rowc,dipz_row_col)
deallocate(kine_val,kine_rowc,kine_row_col)
deallocate(moq1_val,moq1_rowc,moq1_row_col)
deallocate(moq2_val,moq2_rowc,moq2_row_col)
deallocate(pot_val,pot_rowc,pot_row_col)
...
open(unit=20,file='d_csr_vectors_NAC',status='unknown')
read(20,'(i12)')k_nac
allocate(nac_val(0:k_nac-1),nac_rowc(0:n), nac_row_col(0:k_nac-1,0:1))
do i=0,k_nac-1
  read(20,'(e23.15e3,2i12)')nac_val(i),nac_row_col(i,0),nac_row_col(i,1)
...
open(unit=20,file='d_csr_vectors_dip',status='unknown')
read(20,'(i12)')k_dipx
allocate(dipx_val(0:k_dipx-1),dipx_rowc(0:n), dipx_row_col(0:k_dipx-1,0:1))
do i=0,k_dipx-1
  read(20,'(e23.15e3,2i12)')dipx_val(i),dipx_row_col(i,0),dipx_row_col(i,1)
...
open(unit=20,file='d_csr_vectors_kin',status='unknown')
read(20,'(i12)')k_kine
allocate(kine_val(0:k_kine-1),kine_rowc(0:n), kine_row_col(0:k_kine-1,0:1))
do i=0,k_kine-1
  read(20,'(e23.15e3,2i12)')kine_val(i),kine_row_col(i,0),kine_row_col(i,1)
...
open(unit=20,file='d_csr_vectors_pot',status='unknown')
read(20,'(i12)')k_pot
allocate(pot_val(0:k_pot-1),pot_rowc(0:n), pot_row_col(0:k_pot-1,0:1))
do i=0,k_pot-1
  read(20,'(e23.15e3,2i12)')pot_val(i),pot_row_col(i,0),pot_row_col(i,1)
...
open(unit=20,file='d_csr_vectors_anm',status='unknown')
read(20,'(i12)')k_am
allocate(am_val(0:k_am-1),am_rowc(0:n), am_row_col(0:k_am-1,0:1))
do i=0,k_am-1
  read(20,'(e23.15e3,2i12)')am_val(i),am_row_col(i,0),am_row_col(i,1)
...
open(unit=20,file='d_csr_vectors_dq1',status='unknown')
read(20,'(i12)')k_moq1
allocate(moq1_val(0:k_kine-1),moq1_rowc(0:n), moq1_row_col(0:k_moq1-1,0:1))
do i=0,k_moq1-1
  read(20,'(e23.15e3,2i12)')moq1_val(i),moq1_row_col(i,0),moq1_row_col(i,1)
...
open(unit=20,file='d_csr_vectors_dq2',status='unknown')
read(20,'(i12)')k_moq2
allocate(moq2_val(0:k_moq2-1),moq2_rowc(0:n), moq2_row_col(0:k_moq2-1,0:1))
do i=0,k_moq2-1
  read(20,'(e23.15e3,2i12)')moq2_val(i),moq2_row_col(i,0),moq2_row_col(i,1)
...
if ( e_ip == 13.0_dp/27.21138628_dp ) then
  ...
  if     ( int(e_ip*27.21138628_dp) == 13 ) then
    fname1 = 'singular_values_09d'
    fname2 = 'U_vec_09d'
    write(*,'(a)')'Ionizing electric field corresponding to the 09th harmonic - DEUTERIUM'
  elseif ( int(e_ip*27.21138628_dp) == 17 ) then
    fname1 = 'singular_values_11d'
    fname2 = 'U_vec_11d'
    write(*,'(a)')'Ionizing electric field corresponding to the 11th harmonic - DEUTERIUM'
  elseif ( int(e_ip*27.21138628_dp) == 20 ) then
    fname1 = 'singular_values_13d'
    fname2 = 'U_vec_13d'
    write(*,'(a)')'Ionizing electric field corresponding to the 13th harmonic - DEUTERIUM'
  elseif ( int(e_ip*27.21138628_dp) == 15 ) then
    fname1 = 'singular_values_15'
    fname2 = 'U_vec_15'
    write(*,'(a)')'Ionizing electric field of 15 eV'
...
do ppp=1,3  !nsamples    <<<<<====================----------------==================----------------===============----------------================
  ...
  auto_d = dcmplx(0.0_dp,0.0_dp)

!$omp parallel do reduction(+:diag_dr,auto_d)
  do i=0,nfiles
    do j=0,n-1
      auto_d(i) = auto_d(i) + (dconjg(wfout(j,0)) * wfout(j,i))
      diag_dr(i) = diag_dr(i) + dconjg(wfout(j,0))*wfout(j,i) * ((wfout(j,0))*dconjg(wfout(j,i))) * eig_val(ppp)
    end do
  end do
!$omp end parallel do

!$OMP parallel do reduction(+:auto_dr_0,auto_dr_1,auto_dr_2,auto_dr_01,auto_dr_02,auto_dr_12)
  do i = 0,nfiles
    do j=0,s-1
      do k=0,s-1
        if ( j /= k ) then
auto_dr_01(i) = auto_dr_01(i) + wfout(0*s+j,0) * dconjg(wfout(0*s+j,i)) * dconjg(wfout(1*s+k,0)) * wfout(1*s+k,i) * eig_val(ppp) + &
wfout(1*s+j,0) * dconjg(wfout(1*s+j,i)) * dconjg(wfout(0*s+k,0)) * wfout(0*s+k,i) * eig_val(ppp)
auto_dr_02(i) = auto_dr_02(i) + wfout(0*s+j,0) * dconjg(wfout(0*s+j,i)) * dconjg(wfout(2*s+k,0)) * wfout(2*s+k,i) * eig_val(ppp) + &
wfout(2*s+j,0) * dconjg(wfout(2*s+j,i)) * dconjg(wfout(0*s+k,0)) * wfout(0*s+k,i) * eig_val(ppp)
auto_dr_12(i) = auto_dr_12(i) + wfout(1*s+j,0) * dconjg(wfout(1*s+j,i)) * dconjg(wfout(2*s+k,0)) * wfout(2*s+k,i) * eig_val(ppp) + &
wfout(2*s+j,0) * dconjg(wfout(2*s+j,i)) * dconjg(wfout(1*s+k,0)) * wfout(1*s+k,i) * eig_val(ppp)
auto_dr_0(i)  = auto_dr_0(i)  + wfout(0*s+j,0) * dconjg(wfout(0*s+j,i)) * dconjg(wfout(0*s+k,0)) * wfout(0*s+k,i) * eig_val(ppp)
auto_dr_1(i)  = auto_dr_1(i)  + wfout(1*s+j,0) * dconjg(wfout(1*s+j,i)) * dconjg(wfout(1*s+k,0)) * wfout(1*s+k,i) * eig_val(ppp)
auto_dr_2(i)  = auto_dr_2(i)  + wfout(2*s+j,0) * dconjg(wfout(2*s+j,i)) * dconjg(wfout(2*s+k,0)) * wfout(2*s+k,i) * eig_val(ppp)
        else
auto_dr_01(i) = auto_dr_01(i) + wfout(0*s+j,0) * dconjg(wfout(0*s+j,i)) * dconjg(wfout(1*s+k,0)) * wfout(1*s+k,i) * eig_val(ppp) + &
wfout(1*s+j,0) * dconjg(wfout(1*s+j,i)) * dconjg(wfout(0*s+k,0)) * wfout(0*s+k,i) * eig_val(ppp)
auto_dr_02(i) = auto_dr_02(i) + wfout(0*s+j,0) * dconjg(wfout(0*s+j,i)) * dconjg(wfout(2*s+k,0)) * wfout(2*s+k,i) * eig_val(ppp) + &
...
!$OMP end parallel do
  ...
  do i=0,nfiles
    auto_dr(i) = auto_dr(i) + dconjg(auto_d(i)) * auto_d(i) * eig_val(ppp)
  end do
...
open(unit=960,file='autoc_d.txt'   ,status='unknown')
open(unit=961,file='diago_d.txt'   ,status='unknown')
open(unit=962,file='cross_d_01.txt',status='unknown')
open(unit=963,file='cross_d_02.txt',status='unknown')
open(unit=964,file='cross_d_12.txt',status='unknown')
open(unit=965,file='cross_d_0.txt' ,status='unknown')
open(unit=966,file='cross_d_1.txt' ,status='unknown')
open(unit=967,file='cross_d_2.txt' ,status='unknown')
do i = 0,nfiles
  write(960,'(e20.10e3)')auto_dr(i)
  write(961,'(e20.10e3)')diag_dr(i)
  write(962,'(e20.10e3)')auto_dr_01(i)
  write(963,'(e20.10e3)')auto_dr_02(i)
  write(964,'(e20.10e3)')auto_dr_12(i)
  write(965,'(e20.10e3)')auto_dr_0(i)
  write(966,'(e20.10e3)')auto_dr_1(i)
  write(967,'(e20.10e3)')auto_dr_2(i)
end do
close(unit=960)
close(unit=961)
close(unit=962)
...
, NormT-1   ,   Ltot    '
write(15,'(17(e17.8e3))') t0,Et,fa(0),fb(0),fc(0),fd(0),fe(0),ff(0),fg(0),fh(0),fi(0),fj(0),fk(0),&
fl(0),fm(0),fn(0),fo(0)
