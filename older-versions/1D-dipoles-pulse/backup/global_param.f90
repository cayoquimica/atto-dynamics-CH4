...
real(kind=8) :: pot1(Nq2,Nq1),pot2(Nq2,Nq1),pot3(Nq2,Nq1) !Matrices with each state potential energy
...
subroutine pot
implicit none
open(unit=1,file='e1.txt',status='old')
open(unit=2,file='e2.txt',status='old')
open(unit=3,file='e3.txt',status='old')
do i=1,Nq1
  read(1,*) pot1(i,:)
  read(2,*) pot2(i,:)
  read(3,*) pot3(i,:)
