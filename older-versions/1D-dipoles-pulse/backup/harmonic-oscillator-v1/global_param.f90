module global_param
implicit none
integer, parameter :: dp1 = kind(1.d0) 
integer i,j,k
integer, parameter :: NA=2 !Number of atoms
integer, parameter :: st=1 !number of electronic states
integer, parameter :: Nq1=101 !number of points of the grid along q1
integer, parameter :: Nq2=61 !number of points of the grid along q2
real(kind=dp1), parameter :: saw2au=1822.889950851334d0 !mass conversion factor from Standard atomic weight to atomic units
real(kind=dp1), parameter :: car=12.011d0*saw2au !Carbon mass in atomic units
real(kind=dp1), parameter :: nitro=14.0067d0*saw2au !Carbon mass in atomic units
real(kind=dp1), parameter :: mredu=nitro**2.0d0/(2.0d0*nitro) ! reduced mass of N2
real(kind=dp1), parameter :: hi=1.00794d0*saw2au !Hidrogen mass in atomic units
real(kind=dp1) :: mass(3*NA),q1(3*NA),q2(3*NA)
real(kind=dp1) :: pot1(Nq1) !Matrices with each state potential energy
