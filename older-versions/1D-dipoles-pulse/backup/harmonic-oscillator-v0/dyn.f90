...
real(kind=8) :: tt,teta,phi,const,ch,x,x0,stepX,kf,w,expo,c0,c1
complex(kind=8) ::term1,term2,vec0(Nq1),vec1(Nq1)
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
w=sqrt(kf/mredu)
a=sqrt(2/(mredu*w))
teta=( atan( (2*tt)/(a**2.d0*mredu) ) ) /2.d0
phi=-teta-(k0**2.d0/(2.d0*mredu))*tt
...
do i=1,Nq1
  ch=(i-(Nq1-1)/2-1)*stepX !change in x
  x=x0+ch ! x in atomic units
!write(3,'(3(f24.15))')x,0.5d0*kf*(x-x0)**2.0d0,e0
write(4,'(f24.15)') 0.5d0*kf*(x-x0)**2.0d0
!  term1 = exp(im*phi)/(a**4.d0+(4.d0*tt/mredu**2.d0))**(1.d0/4.d0) * exp(im*k0*(x-x0))
!  term2 = exp( - ((x-x0) - (k0*tt/mredu))**2.d0 / (a**2.d0 + (2*im*tt/mredu)) )
!  vec0(i) = const * term1 * term2
  expo = dexp(-(x-x0)**2.d0*mredu*w/2.d0) !* exp(im*k0*(x-x0))
  vec0(i) = c0 * expo
  vec1(i) = c1 * (x-x0) * expo
!  vec0(i)=(2.d0/(a**2*pi))**(1.d0/4.d0) * exp( (im*k0*(x-x0)) ) * exp(-(x-x0)**2.d0/a**2.d0)  !16 is the position of the minimum in q1 coordinate
...
integer i,k,n
real(kind=8) :: t0,tf,tt(npoints),h,t,coordinate(n),stepX
complex(kind=8) :: soma,dydt(n),y(n),y0(n),ymatrix(n,npoints+2),ymatlab(n,npoints),Te,im,aux1,aux2
external derivs,rk4 !
im = dcmplx(0.d0,1.d0)
...
implicit none
integer n
real(kind=8) :: Ha(n,n),t,totmass
complex(kind=8) :: y(n),dydt(n),kine(n),im,cm(n)
...
use global_param
implicit none
real(kind=8) :: Ha(Nq1,Nq1),ind1,ind2,ind3,step,const
