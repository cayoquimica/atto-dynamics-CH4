...
complex(kind=dp) ::vec0(st*Nq1),vec1(st*Nq1),vec2(0:st*Nq1-1),vec3(0:st*Nq1-1)
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
call hamiltonian
n=st*Nq1 ! m=Nq1*Nq2*st %size of the Hamiltonian

t0=0.d0
tf=100000.d0 !40 = Approx. 1 femtosecond
npoints=1000000.d0 !If 1000, the step will be approx. 1 attosecond
...
call derivs(tt,vec2,n,vec3,tstep) !evaluating y'(t=0,y)!
soma=0.0d0                                               !
do i=1,Nq1                                               !
  soma=soma + dconjg(vec2(i)) * vec3(i)*im*stepX
end do                                                   !
!--------------------------------------------------------!
...
use global_param
integer n
real(kind=dp) :: t0,tf,npoints,tt(npoints),h,t,coordinate(0:n-1),soma,Te,aux1,aux2
complex(kind=dp) :: dydt(0:n-1),y(0:n-1),y0(0:n-1),ymatrix(0:n-1,npoints+2),ymatlab(0:n-1,npoints)
...
do1: do i=1,n
  y(i-1)=y0(i-1) !loading initial conditions
  ymatrix(i-1,2)=y(i-1) !saving initial amplitudes
end do do1
...
Te=0.d0                                   !
do i=1,n                                  !
  soma=soma+dconjg(y(i-1))*y(i-1)*stepX       !
  Te=Te + dconjg(y(i-1)) * dydt(i-1)*im *stepX!
end do                                    !
!-----------------------------------------!
aux1=soma
aux2=Te
...
!write(*,'(a14,f23.15)')'current time =',t
  ...
  do2: do i=1,n
    ymatrix(i-1,k+2)=y(i-1) !saving the amplitude for time t
  end do do2
  t=t+h
...
st3p(k)=dconjg(y(n-1))*y(n-1)*stepX                       !
!-----------------------------------------------------!
end do do3
!---------------------------------------------------------------------------------------------------------------------------!
...
write(*,*) '************************************************************'
do i=1,Nq1
  ymatrix(i-1,1)=(i-(Nq1-1.d0)/2.d0-1.d0)*stepX
!  ymatrix(i,1)=i
  coordinate(i-1)=ymatrix(i-1,1)
end do
...
write(50,'(es26.16e3)') Et
  do i=1,Nq1
  write(20,'(4(es26.16e3))') coordinate(i-1),real(dconjg(ymatrix(i-1,j+1))*ymatrix(i-1,j+1)*stepX),&
real(dconjg(ymatrix(Nq1+i-1,j+1))*ymatrix(Nq1+i-1,j+1)*stepX),&
real(dconjg(ymatrix(2*Nq1+i-1,j+1)) * ymatrix(2*Nq1+i-1,j+1) *stepX)
    write(30,'(4(es26.16e3))')coordinate(i-1),(real(ymatrix(i-1,j+1))),(real(ymatrix(Nq1+i-1,j+1))),(real(ymatrix(2*Nq1+i-1,j+1)))
    write(40,'(4(es26.16e3))')coordinate(i-1),(aimag(ymatrix(i-1,j+1))),(aimag(ymatrix(Nq1+i-1,j+1))),(aimag(ymatrix(2*Nq1+i-1,j+1)))
  end do
  write(20,*)''
  write(20,*)''
  write(30,*)''
...
do1: do i=1,n
  yt(i-1) = y0(i-1) + (h/2.d0) * dydt(i-1) !Evaluation of y(t) + k1/2.0d0
end do do1
call derivs(th,yt,n,dyt,h/2.d0) !dyt = k2/h ---- Evaluation of y'(t + h/2.0d0 , y(t) + k1/2.0d0)
do2: do i=1,n
  yt(i-1) = y0(i-1) + (h/2.d0) * dyt(i-1)  !Evaluation of y(t) + k2/2.0d0
end do do2
call derivs(th,yt,n,dym,h/2.00) !dym = k3/h ---- Evaluation of y'(t + h/2.0d0 , y(t) + k2/2.0d0)
do3: do i=1,n
  yt(i-1) = y0(i-1) + h * dym(i-1) !Evaluation of y(t) + k3
  dym(i-1) = dyt(i-1) + dym(i-1)  ! making dym = (k2 + k3)/h ---- variable dyt free now
end do do3
call derivs(t+h,yt,n,dyt,h*2.d0) !dyt = k4/h ---- Evaluation of y'(t + h/2.0d0 , y(t) + k1/2.0d0)
do4: do i=1,n
  Yout(i-1) = y0(i-1) + h/6.d0 * (dydt(i-1) + 2.d0 * dym(i-1) + dyt(i-1))
end do do4
end subroutine rk4
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
!    dydt(i) = dydt(i) + ( Ha_val(j) + (dip_val(j) * Et) ) * y(Ha_row_col(j,1)) / im
    dydt(i) = dydt(i) + ( Ha_val(j) ) * y(Ha_row_col(j,1)) / im
  end do
end do
!$OMP end parallel do
