...
integer,parameter                          :: q1i0=27!-15
integer,parameter                          :: q2i0=20!-8
...
integer,parameter                          :: nsamples = 1000*8 !Number of randon orientations to sample for the photoionization around each bond (the *4 is for each bond)
...
real(kind=dp),parameter                    :: hi=2.01410177811_dp*saw2au !Hydrogen mass in atomic units
...
real(kind=dp),parameter                    :: e_ip =11.0_dp * fre_800 !Energy of the ionizing pulse in atomic units approx 21.22 ev - the Helium ressonance band
...
do j=1,Nq2
  do i=1,Nq1
    ...
    pice(k)     = - dsqrt(2.d0) * 0.05d0 * dot_product( orientation , phote(i,j,1,:) )
    pice(k+s)   = - dsqrt(2.d0) * 0.05d0 * dot_product( orientation , phote(i,j,2,:) )
    pice(k+2*s) = - dsqrt(2.d0) * 0.05d0 * dot_product( orientation , phote(i,j,3,:) )
!    k = k + 1
  end do
end do
!$OMP end parallel do
